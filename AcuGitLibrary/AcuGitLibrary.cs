...
namespace AcuGitLibrary
{
    ...
    public class Git
    {
            ...
            using (var repo = new LibGit2Sharp.Repository(repoPath))
            {
                ...
                if (last != null) {
                    ...
                    LibGit2Sharp.CherryPickResult result = repo.CherryPick(commit, sign);
                        ...
                        //The program doesn't take this action or the following ... likely gets stuck cause im dumb and left commit instead of putting in C
                    foreach (LibGit2Sharp.Commit C in MasterCommits) {
                        LibGit2Sharp.CherryPickResult r = repo.CherryPick(commit, sign);
                        try
                        {
                            if (r.Commit == null)
                            {
                                //clean up index if there is a failed pick, which there should be at least 1
                                Reset(ResetMode.Hard);
                            }
                        }
                        catch (Exception e) { }
         ...
         *     1. Add in a Revert Command to undo commits
         *     2. A simple delete branch command
         *
         */ 
    }
    /// <summary>
